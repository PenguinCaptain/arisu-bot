...
if __name__ == "__main__":
    ...
    answer, display = generate_question(["arcaea"], num)

    song_guess_data = generate_dict(answer, display, "", {}, 0, {})

    uid = 114514

    print(song_guess_data)
    ...
    while True:
        ...
        song_guess_data, resp = guess(response[1], song_guess_data, response[2], uid)

        if resp == -1:
            print("这个字母已经猜过了，不用再猜一遍")
        elif resp == -2:
            print("或许输入的指令有误，请更正后再试一遍")
        else:
            print(song_guess_data)

        if check_game_end(song_guess_data) == True:
            break
