...
response = requests.get(
    redirect_page, cookies=cookies, allow_redirects=False)
cookies = response.cookies
...
def parse(data):
    ...
    for lev in ['basic', 'advanced', 'expert', 'master']:
        ...
        for single_list in music_box:
            title = re.findall(
                r'div class="music_title">.*?</div>', single_list)[0]
            title = re.findall(r'>.*?<', html.unescape(title))[0][1:-1]
            score = re.findall(r'<span class="text_b">.*?</span>', single_list)
            music_idx = re.findall(
                r'<input type="hidden" name="idx" value=".*?" />', single_list)[0][39:-4]
            if len(score) == 0:
                score = 0
            else:
            ...
            if int(music_idx) not in idx2img_url:
                requests.post('https://chunithm-net-eng.com/mobile/record/musicGenre/sendMusicDetail/',cookies=cookies, data={'idx': music_idx, 'token': cookies["_t"]})
                response = requests.get(
...
def job_request(lev):
    ...
    extract_datas += [(output_lev, *row) for row in result if int(row[-2]) != 0]
